...
def launch_labelimg():
    ...
    try:
        ...
        command = ["labelImg", os.path.abspath(RAW_DATA_DIR), os.path.abspath(CLASSES_FILE)]
...
def launch_training():
    ...
    print("--- 准备启动训练流水线 ---")
    try:
        command = ["python", "train_yolo.py", "--data-dir", RAW_DATA_DIR, "--classes-file", CLASSES_FILE]
        print(f"执行命令: {' '.join(command)}")
